...
public class BreakfastAgentSimple : Agent {
	...
	public override void AgentStep(float[] act)
	{
		...
		reward += (3 - (panCenter.position - food.position).sqrMagnitude)*0.01f;
	...
	void MoveLimbs(float[] act)
	{
        ...
        float torque_z = Mathf.Clamp(act[0], -1, 1) * torqueStrengthMult;
        ...
        float torque_x = Mathf.Clamp(act[1], -1, 1) * torqueStrengthMult;
        limbs[1].GetComponent<Rigidbody>().AddTorque(limbs[1].right * torque_x);
